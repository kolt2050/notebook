...
- **Блоки кода с подсветкой синтаксиса**:
  - Инлайн-код: оберните текст в обратные кавычки `` `код` ``.
  - Блок кода: введите ` ``` ` или ` ```python ` и нажмите **Enter**.
  - Поддержка подсветки синтаксиса для множества языков (Python, JavaScript, HTML, CSS, Java, C++ и др.).
...
| :--- | :--- |
| **Инлайн-код** | Оберните текст в обратные кавычки: `` `код` `` |
| **Создать блок кода** | Введите ` ``` ` или ` ```python ` + **Enter** |
| **Новая строка в блоке** | **Enter** |
| **Выход из блока** | **Shift + Enter** (подсветка синтаксиса применяется автоматически) |
| **Вставка кода** | **Ctrl + V** внутри блока |

Поддерживаемые языки: `python`, `javascript` (`js`), `html`, `css`, `java`, `cpp`, `csharp`, `go`, `rust`, `sql`, `bash`, `json`, `xml`, `yaml` и [многие другие](https://highlightjs.org/static/demo/).
...
Для пользователей Windows предусмотрен скрипт автоматической сборки и запуска:
1. Запустите файл `windows_app.bat`.
2. Скрипт соберет Docker-образ.
3. Окно терминала автоматически закроется после успешного запуска.
4. Браузер откроется автоматически через 5 секунд.
Если произойдет ошибка сборки, окно останется открытым, чтобы вы могли прочитать сообщение об ошибке.
